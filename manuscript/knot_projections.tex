...
\begin{itemize}
\item[1.] Gaussian model: \(P\in\Rbb^{3\times N}\) has independent
entries distributed as \(\mathcal N(0,1/3)\), and the polygon is
\(\Gamma(P;\mathcal C)\).
\item[2.] Haar row-orthonormal model: \(Q\) is Haar distributed on
\[
...
are separated from topology: Gaussian projections add singular-value shear, while the
...
either the Gaussian model or the Haar row-orthonormal model, the corresponding random
...
\Ebb\|P(e_i-e_j)\|^2=2 .
\]
In particular, the Gaussian normalization preserves the squared simplex distance
...
the planar polygon obtained from the first two coordinates of the
...
are compared:
\begin{itemize}
\item[1.] Gaussian model: sample \(P_{ab}\sim\mathcal N(0,1/3)\), center by
\(P\Pi_H\), and use the translated-equivalent vertices \(Pe_i\);
\item[2.] Haar row-space model: sample a Haar three-plane in \(H\) and choose
...
\(p_N(K)\).  The Gaussian run also records the metric shear retained by \(P\Pi_H\);
...
\end{tabular}
\caption{Empirical resolved-nontrivial counts for the Haar row-space and Gaussian
...
The Wilson intervals for the direct Haar and Gaussian counts in
...
\]
The original simplex distance is \(\sqrt2\).  By Lemma~\ref{lem:raw-distance-law}, the Gaussian normalization has expected squared pair distance \(2\).  By
...
Gaussian centering invariance, \(2(N-3)\) sensitivity, and circuit/intersection
